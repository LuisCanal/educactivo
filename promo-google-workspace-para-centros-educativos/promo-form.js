(function () {
  var form = document.getElementById("educactivo-promo-form");
  if (!form || !(form instanceof HTMLFormElement)) return;

  var statusEl = document.getElementById("educactivo-promo-status");
  var submitBtn = form.querySelector('button[type="submit"]');
  var qEl = document.getElementById("promo-captcha-q");
  var tokenInput = document.getElementById("promo-captcha-token");
  var answerInput = document.getElementById("promo-captcha");
  ...
  form.addEventListener("submit", function (e) {
    ...
    var payload = {
      ...
      user_count: String(fd.get("user_count") || "").trim(),
    ...
    fetch("/api/contact", {
      ...
      .then(function (result) {
        if (result.ok && result.data && result.data.ok) {
          if (statusEl) {
            statusEl.textContent = "Gracias, tu solicitud fue enviada.";
